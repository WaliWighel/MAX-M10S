...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    switch (GPIO_Pin) {
        case GPS_EXTI_Pin:
            if (MAX_M10S_HandleEvent(EVENT_START_CYCLE) != MAX_M10S_OK) {
                return;
            }
            break;
    }
...
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *hi2c) {
    switch ((uint32_t)hi2c->Instance) {
        case (uint32_t)I2C2:
            if (MAX_M10S_HandleEvent(EVENT_I2C_RX_DONE) != MAX_M10S_OK) {
                return;
            }
            break;
    }
...
static void MPU_Config(void)
{
  ...
  */
  ...
  MPU_InitStruct.BaseAddress = 0x2404FC00;
